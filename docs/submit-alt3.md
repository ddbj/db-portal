...
研究者は手元に **ファイル** を持っている (FASTQ、FASTA、VCF、CEL、mzML、AGP、GFF、表現型 table、…)。これをそのままテーブルに並べてもらい、各ファイルの属性 (組織 / 公開可否 / データ形態 など) を per-cell で編集してもらう。テーブルの状態が確定するほど、ポータル側は **「あなたの登録は次の順序で進めてください」** を詳細化していく。

「自分のデータの DB は何か」や「自分の研究の性質はどれか」のような **抽象的な自己認識** をユーザーに要求しない。研究者は具体物 = ファイルから入って、各ファイルの属性を埋めるほど登録経路が詳細化される。
...
- **MGA (Mass sequence for Genome Annotation)**: 新規受付終了済み (ddbj/www `_ddbj/data-categories.md`)。既存データの閲覧は DDBJ サイトから引き続き参照可能で、ポータルからの新規登録経路は提供しない
