...
| 項目 | 設定値 |
|---|---|
| Valid redirect URIs | `http://localhost:3000/*`, `https://portal-dev.ddbj.nig.ac.jp/*` |
| Valid post logout redirect URIs | `http://localhost:3000/*`, `https://portal-dev.ddbj.nig.ac.jp/*` |
| Web origins | `http://localhost:3000`, `https://portal-dev.ddbj.nig.ac.jp` |

**Production:**

| 項目 | 設定値 |
|---|---|
| Valid redirect URIs | （本番ドメイン確定後に設定） |
| Valid post logout redirect URIs | （本番ドメイン確定後に設定） |
| Web origins | （本番ドメイン確定後に設定） |
...
API がある場合、JWT の `aud` クレーム検証に必要。第一段階では API 検証がないため任意だが、設定しておくことを推奨する。
