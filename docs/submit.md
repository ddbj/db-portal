...
カードの表示優先順は上から順。第一段階で全 8 カードの詳細パネルを書き下ろす。
...
詳細パネルの記述深度はカードごとに調整する（ゲノム系は最も詳しく、外部リダイレクト系（変異・プロテオミクス）は概要 + 外部リンク中心）。

### 登録窓口への導線（現状と将来）

第一段階では、詳細パネルから既存の登録窓口（D-way, MSS フォーム, NSSS 等）への外部リンクで誘導する。
...
| URL | 用途 | レンダリング |
|---|---|---|
| `/submit` | 登録ナビゲーション（Section A / B / C を 1 ページに縦積み） | プリレンダ |

`/submit` は SEO ターゲットであり、静的コンテンツが中心（Section A/B/C ともに TSX で書き下ろし、フローチャートも `@xyflow/react` の SSR 対応を使う）なのでプリレンダで配信する。

### 第一段階（2026/4 モック時点）

クエリパラメータなし。Section A のカード選択・Section B のフロー進行・Section C の表示内容はクライアントサイドの UI state のみで管理する（URL に反映しない）。

理由:

- モック時点では遷移の複雑さを抑える
- フロー進行状態（複数選択肢 × 3 階層）を URL にシリアライズする方式が定まっていない
- 第一段階のユーザーは「トップから `/submit` を開いて該当カードをクリック」の動線で足りる

### 将来拡張（リリース後フェーズ）

ブックマーク / 共有のため、Section A の選択状態のみ URL に載せる:

| パラメータ | 値 | 用途 |
|---|---|---|
| `for` | ユースケース ID（ケバブケース） | Section A のカード選択を復元し、同時に Section B の該当経路をハイライト、Section C に該当パネルを表示 |
...
- **Section B のフロー進行状態**（`?flow=q1-no,q2-eukaryote,q3-raw-assembly` 等）: (1) 3 階層・12 ゴールで URL がすぐ壊れる、(2) Section A/B/C は相互連動しているので `for` だけあれば Section B のハイライトと Section C の切替は再現可能、(3) フロー進行の途中状態を共有したいニーズは薄い（完走後のゴール共有なら `for` で十分）
- **Section C の開閉状態**: インライン展開で常時描画しているため状態不要
...
- [ ] 詳細パネルの具体的な記述内容（全 8 カード分）。外部リダイレクト系ゴール（変異データの JVar / EVA / dgVa、プロテオミクスの jPOST）の誘導 UI も本項目に吸収する: 「概要 + 外部リンク中心」方針（本章「詳細パネル」節参照）に従い、Callout で外部遷移である旨を明示したうえで `Button variant="secondary"` または `TextLink external` で新規タブ遷移させる形を基本とする。自動リダイレクトや即時 `window.open` は行わない
- [x] Section C の UI 形態: **インライン展開**（Section B の直下に常時描画、同一領域で中身を差し替え）を採用。決め手は (1) `/submit` の SSR / プリレンダリング方針と素の DOM 描画の相性、(2)「Section C は同一領域でコンテンツだけ差し替え」方針との直結、(3) focus trap / ESC / scroll lock 等を自前実装せずに済む a11y・モバイル両面での素直さ。サイドパネル・モーダルは SSR 相性と 1 ページ完結方針との不整合で却下、アコーディオンは A/B → C 自動切替と開閉状態の二重管理で UX が複雑化するため却下。切替時は Section C 見出しに `scrollIntoView({ behavior: "smooth" })`、初期状態は 1 枚目（微生物ゲノム）のパネルを表示しておく。Section B のスティッキー化は本決定と独立した別論点として持ち越し
- [ ] Section B（フローチャート）のスティッキー化: Section C にスクロールした際に上部へ残留させるかどうか。`@xyflow/react` のフローは縦幅が大きく、スティッキー化すると C の登録フロー図・ステップ説明を読む際に上部が圧迫される懸念がある。**モック実装後に実機で触りながら決める**（スティッキー有無・高さ縮小表示・折りたたみ等の選択肢を実装上で比較）
- [x] フローチャート進行状態の URL 反映可否: URL 設計の包括検討で決定（本章「URL 設計」節参照）。第一段階はクエリなし、将来拡張で Section A のカード選択のみ `?for=<usecase-id>` で反映、Section B のフロー進行（`?flow=...`）と Section C の開閉状態は却下。決め手は (1) `for` だけで A/B/C の表示は再現可能、(2) `flow` は 3 階層 12 ゴールで URL がすぐ壊れる、(3) 共有したい状態は「完走後のユースケース」であって途中状態ではない
- [x] フローチャートの描画ライブラリ選定: `@xyflow/react` v12 系（React Flow）を採用。決め手は SSR 公式サポート（React Router v7 のプリレンダリング方針と整合）、カスタムノードで react-i18next / Tailwind を自然に適用できること、カード経路ハイライトを `nodes` / `edges` の className 切替で宣言的に実装できること。Mermaid は SSR 不可・i18n 相性不良・バンドル肥大で却下。自前 SVG は描画品質で勝るがエッジ配線・モバイル・a11y の自作工数がマイルストーンに合わず却下
- [x] 複数ユースケースに該当する場合の案内方法: 第一段階では専用 UI を作らない。Section A / B は単一選択のまま、Section C にも関連カード切替リンク等の切替導線を置かない。必要なカード（ゲノム系 3 枚: 微生物ゲノム / 真核生物ゲノム / メタゲノム）の詳細パネル内で BioProject umbrella 統合の概念に平文で触れる程度に留める。複数 DB にまたがる submission set の統合案内は、将来の [DDBJ Record](https://github.com/ddbj/ddbj-record-specifications) ベース登録 UI（本章「登録窓口への導線（現状と将来）」、overview.md「将来的な拡張」参照）が SSOT となる。今の段階で切替型 UI を作り込むと情報が各カード末尾に分散し、Record UI への移行時に書き直し負債になるため避ける
