...
> v2 評価並走中: `/submit-alt` で質問ウィザード起点の代替仕様 ([submit-alt.md](./submit-alt.md)) を並走評価中。トップ導線・本流選択は未確定。
...
**インライン展開**（Decision Tree の直下に常時描画、同一領域で中身を差し替え）を採用。決め手は SSR / プリレンダリング方針との相性、同一領域コンテンツ差替え方針との直結、focus trap / ESC / scroll lock 等を自前実装せずに済む a11y・モバイル両面での素直さ。

初期状態（`?for=` なしで `/submit` を開いた時）は未選択。Detail Panel には空状態プレースホルダ（「ユースケースを選ぶと詳細が表示されます」）を表示し、カードも Decision Tree も非ハイライト状態で描画する。ページは上端から開く。

スクロール挙動は操作元で分ける。**カードクリック時**のみ Detail Panel 見出しに `scrollIntoView({ behavior: "smooth", block: "start" })` で遷移し、**Decision Tree の node クリック時**はスクロールしない（tree を辿る途中で視点が毎回動くのを避けるため）。URL 直打ち / リロード / `?for=xxx` 付きアクセス時もスクロールしない。
...
- `/submit`: 初期状態（未選択、Detail Panel は空状態プレースホルダ）
...
- **詳細パネルの UI 形態**: インライン展開。Decision Tree の直下に常時描画、同一領域で中身を差し替え。サイドパネル・モーダルは SSR 相性と 1 ページ完結方針との不整合で却下、アコーディオンはカード / tree → Detail Panel 自動切替と開閉状態の二重管理で UX が複雑化するため却下。**初期状態（`?for=` なし）は未選択で空状態プレースホルダを表示**し、ページは上端から開く。**スクロールはカードクリック時のみ**発火させ、Decision Tree の node クリック時は発火させない（tree を辿る操作ごとに視点が動くのを避けるため）
