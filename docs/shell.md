...
`app/shell/` は `app/ui/` の primitive と `app/lib/` の hook / helper を消費する。`app/features/` を import してはならない (`architecture.md` zones)。

`app/shell/` は ESLint `react/forbid-elements` から除外される (`docs/ui-primitives.md`)。LoginButton / Footer / SwitchLang 等は BFF endpoint や外部 URL への `<a href>` を直接扱うため。`<button>` 等を書く場合も primitive 経由を優先し、必要なときだけ生要素にとどめる。生 hex / arbitrary Tailwind value 禁止は `app/shell/` でも引き続き効く。
...
背景は `surface` (白)、下に `1px solid border-soft` の境界 (`design/constraints.md` で許容)。紫ベタ / グラデーション / 上端帯は使わない。
...
returnTo はクライアントから渡す。`buildLoginUrl` / `buildLogoutUrl` が同一 origin 検証して `/` 始まり以外を `/` に正規化する (`docs/auth.md`)。
...
`useBreadcrumb` は handle resolver パターンで構築される (`docs/content-system.md`)。`app/shell/breadcrumb.tsx` は hook を呼んで `<nav aria-label={t("a11y.breadcrumbNav")}>` で wrap するだけ。
...
`react-i18next` の `fallbackLng: "ja"` により、en リソースに無いキーは ja の値が render される (`docs/i18n.md`)。これにより:
...
- `SkipLink` は Header の **上** に置く sr-only な link で、Tab で focus したときだけ可視化する (`design/constraints.md`)
...
これにより SSR の並列 request で `changeLanguage` のレースが発生しない (`docs/i18n.md`)。module-level の global instance は持たない。
...
ja と en でキーセットは完全一致させる (`docs/i18n.md` PBT で担保)。
