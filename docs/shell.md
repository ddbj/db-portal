...
ja / en で同じ構造、 文言だけ i18n リソースから引く:

| key | i18n key | 遷移先 (ja) | 遷移先 (en) | active 条件 |
|---|---|---|---|---|
| top | `nav.top` | `/` | `/en` | path が `/` または `/en` |
| search | `nav.search` | `/search` | `/en/search` | path が `/search` 始まり (results 含む) |
| submit | `nav.submit` | `/submit` | `/en/submit` | path が `/submit` 始まり |
| news | `nav.news` | `/news` | `/en/news` | path が `/news` 始まり |

`/databases/:slug` は top-level nav に含まない (deep page、active = null)。
...
const isActive = (path: string, current: string, lang: Lang): boolean => {
...
`current` は `useLocation().pathname` から取得する。
