...
### compose.podman.yml (production / staging override)
...
News mirror は git protocol HTTPS で動くため GitHub PAT は不要 (`decisions.md`)。詳細な rotation 手順は `operations.md` を参照。
...
`/api/llm/health` の `status` フィールドの解釈は `llm.md`。production / staging では `unset` は出ない設計 (env に LLM URL が設定されているため)。
