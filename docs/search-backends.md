...
これらは**初期値**。リリース後に実ログのレイテンシ分布を見て調整する。
...
- **初期リリース**: 何もしない。タイムアウト 15s で cold も収まるため実害小。インデックス再オープン直後の最初の数クエリだけ体感で遅い
...
```json
{
  "databases": [
    ...
    { "db": "ddbj", "count": null, "error": "timeout" },
...
- **ARSA の実ワードレイテンシ**: staging 3 shard で cold cache 最大 8.7s（`Homo sapiens`）を実測。タイムアウト値を 15s に設定
- **ES analyzer 変更の可否**: 変更しない方針で確定。フレーズ検索は standard analyzer で正常動作を実測確認。`HIF-1` と `HIF 1` の区別が必要なケースは Advanced Search の identifier 完全一致で救う
- **prod ARSA 8 shard のレイテンシ実測**: 完了。cold cache 最大 `Homo sapiens` 5.59s、`"HIF-1"` 6.67s、warm は全て 10ms 台。タイムアウト 15s で全ケース収まることを確認
- **proxy 起動時 warming**: 初期リリースは実施しない方針で確定（cold でも 15s 以内、warm cache ヒット後は 10ms 台）。負荷状況次第で ddbj-search-api に代表語 warming タスクを追加する選択肢を残す
