...
| BioProject | `child_biosamples` | `properties.Project.ProjectType.ProjectTypeSubmission.Target.BioSampleSet.ID[]` | **要 parse** |
...
現状 `parentBioProjects` / `childBioProjects` は `{type: object, enabled: false}` で検索不可。加えて `properties.Project.ProjectType.ProjectTypeSubmission.Target.BioSampleSet.ID[]` に関連 BioSample の ID が入っている。

**ユースケース例:**
- 「PRJDB12345 配下の全 BioSample」
- 「この親 BioProject に属する全 SRA」

**parse 対象と追加フィールド:**

| 追加フィールド（ES） | parse 対象 path |
|---|---|
| `parentBioProjects[]`（keyword） | 既存フィールドの enable 化 |
| `childBioProjects[]`（keyword） | 既存フィールドの enable 化 |
| `childBioSamples[]` | `properties.Project.ProjectType.ProjectTypeSubmission.Target.BioSampleSet.ID[]` |
...
| BioProject の親子関係 | **将来拡張** | UC7 は accession から直接 DB 詳細ページに飛ぶ動線で部分的に代替可能 |
