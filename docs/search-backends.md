...
```json
{
  ...
  "title": "Unknown field",
  "status": 400,
  "detail": "unknown field 'foo' at column 15 (length 3). allowed: identifier, title, description, organism, ...",
...
**`type` の URI 規約**: `https://ddbj.nig.ac.jp/problems/<slug>` 形式（kebab-case）。slug 自体が machine-readable identifier として機能する（portal 側は `ApiError.slug` getter で抽出）。RFC 7807 §3.1 のとおり URI は dereferenceable である必要はない。位置情報（column / length）は `detail` 文字列に自然言語で埋め込む（API 側 `docs/db-portal-api-spec.md` の方針に揃える）。

**エラー slug 一覧（ddbj-search-api 0.3.0 の SSOT）:**

DSL 関連 7 slug（`/db-portal/cross-search` / `/db-portal/search` / `/db-portal/parse` で共通）:

| slug | HTTP | 発生条件 |
|---|---|---|
| `unexpected-token` | 400 | DSL 構文エラー（Lark `UnexpectedToken` / `UnexpectedCharacters`）。非対応構文（boost `^` / fuzzy `~` / 正規表現 `/.../`）、過長 DSL、空入力もここに該当 |
| `unknown-field` | 400 | allowlist 外のフィールド名 |
| `field-not-available-in-cross-db` | 400 | 横断モード（`db` 未指定）で Tier 3 フィールドを使用。`detail` に候補 DB を列挙 |
| `invalid-date-format` | 400 | 日付値が `YYYY-MM-DD` 形式でない、または存在しない日付 |
| `invalid-operator-for-field` | 400 | フィールドに対して不正な演算子（例: `date:cancer*`、`identifier:[a TO b]`） |
...
- 400: deep paging 超過（`page * perPage > 10000`）、cursor 排他違反、不正な cursor、cursor 期限切れ
- 422: enum / Literal 違反、型不一致（`db` / `sort` / `perPage` 等のパラメータ検証）
- 502: 横断 fan-out で全 DB 失敗、Solr DB 指定検索で upstream エラー

**UI の扱い:**

- portal は `ApiError.slug` から slug を取り出し、`routes.search.errors.<slug>` の i18n key で対応する人間向けメッセージに変換する（[Day 2 で実装済](./search.md#検索結果-ui)）
...
| BioSample / SRA-sample | `geo_loc_name` | `geoLocName`（converter 0.3.0 top-level、BioSample / SRA-sample 共通） | converter top-level 化済 / API allowlist |
| BioSample / SRA-sample | `collection_date` | `collectionDate`（converter 0.3.0 top-level、BioSample / SRA-sample 共通、text 型） | converter top-level 化済 / API allowlist |
| BioSample | `host` | `host`（converter 0.3.0 top-level） | converter top-level 化済 / API allowlist |
| BioSample | `strain` | `strain`（converter 0.3.0 top-level） | converter top-level 化済 / API allowlist |
| BioSample | `isolate` | `isolate`（converter 0.3.0 top-level） | converter top-level 化済 / API allowlist |
...
| Taxonomy | `common_name` / `japanese_name` / `synonym` | TXSearch 同名フィールド | TXSearch 既存（`japanese_name` は staging schema 不在のため allowlist 外） |
| JGA | `study_type` | `studyType`（jga-study、converter 0.3.0 top-level） | converter parse 済 / API allowlist |
...
ポータル用 endpoint は **3 系統** に分かれる（ddbj-search-api 0.3.0 で確定）。詳細は API リポジトリの `docs/db-portal-api-spec.md` を SSOT とし、本書には UI 側から見た契約のみ記載する。

```
GET /db-portal/cross-search?q=<keyword>                # 横断シンプル検索（count + 上位ヒット、8 DB 並列 fan-out）
GET /db-portal/cross-search?adv=<dsl>                  # 横断 Advanced Search（同上、Tier 1/2 のみ）
GET /db-portal/search?db=<id>&q=<keyword>              # DB 指定シンプル検索（hits + ページネーション）
GET /db-portal/search?db=<id>&adv=<dsl>                # DB 指定 Advanced Search（同上、Tier 1/2/3 全許容）
GET /db-portal/parse?adv=<dsl>&db=<id>                 # DSL → AST 構造化 JSON（GUI state 復元用）
```
...
- 8 DB 並列 fan-out + 個別タイムアウト（ES 10s / ARSA 15s / TXSearch 5s）+ 全体 20s
- 部分失敗許容（1 DB 以上成功で 200、全失敗で 502）
- レスポンスは 12-field の `DbPortalLightweightHit` 配列（DB 別追加 field は含めない）
...
- `adv` 必須、`db` 任意（省略で cross モード = Tier 1/2 のみ許可）
- 内部処理は cross-search / search の DSL 分岐と同じ pipeline（`parse` → `validate` → `ast_to_json`）
- レスポンスは `{ ast: BoolOp | LeafValue | LeafRange }` のツリー JSON（`op` 値による discriminated union）
- portal の Advanced Search ページが `?adv=...` 着地時に呼び出して GUI 状態を復元する用途

**ポータル UI の `/search` URL との対応関係:**
...
> **Status（2026-04 時点）**: ddbj-search-converter 0.3.0 で **Tier 2 正規化 / SRA library・platform / JGA studyType・datasetType / GEA experimentType / MetaboBank study/experiment/submissionType / BioSample top-level（host / strain / isolate / geoLocName / collectionDate / model / package）** までの parse は実装済み。下記の節は仕様の経緯と未対応項目を残す。
> ddbj-search-api 0.3.0 で Tier 3 allowlist 全 36 unique field（per-DB 集計 41 field）を公開済み。BioSample 5 / SRA 3 / JGA 2 / BioProject 1 = 11 field の追加 allowlist 化も完了。残課題は BioSample の `attributes.harmonized_name` 経由の検索（`disease` / `tissue` / `env_biome`）、JGA の `principal_investigator` / `submitting_organization`、Taxonomy の `japanese_name`（staging schema 不在）の 3 系統のみで、いずれも将来拡張扱い。
