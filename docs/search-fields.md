...
「filter」は 2 つの別 UI である。Advanced builder は OR / NOT / グルーピングを表せるエディタ (`/search` のみ) で、上部 scope セレクタが供給する DB scope に応じて field 候補を変える (cross = 共通 Tier1/2、単一 DB = 共通 + その DB の DBごと field)。Sidebar は AND of rows で、scope に応じて DBごと field を出す (`/search/results` で唯一の編集可能 filter)。**scope 切替は非破壊**で、scope 外になった既存 condition は live sync (`/db-portal/serialize` を選択 DB scope で呼ぶ) が `field-not-available-in-cross-db` 等で invalid を知らせる。
...
| name (text+keyword)                                | `name`                | ○      | ○ (eq / contains) | text (cross + 6 ES)                 | —                   |
...
| dateCreated (date)                                 | `date_created`        | ○      | ○ (between)       | range (cross + 6 ES)                | —                   |
| organization.name (nested text)                    | `submitter`           | ○      | ○ (eq / contains) | text (6 ES DB)                      | —                   |
| publication.title (nested text)                    | `publication`         | ○      | ○ (eq / contains) | text (cross + 5 ES、biosample 除く) | —                   |
| grant.title (nested text)                          | `grant_title`         | per-DB | ○ (per-DB)        | text (bioproject / jga)             | —                   |
| grant.agency.name (2-level nested)                 | `grant_agency`        | per-DB | ○ (per-DB)        | text (bioproject / jga)             | —                   |
| externalLink.label (nested)                        | `external_link_label` | per-DB | ○ (per-DB)        | text (bioproject / jga)             | —                   |
| type (keyword)                                     | `type`                | per-DB | ○ (per-DB)        | facet (sra / jga)                   | type (sra / jga)    |
| isPartOf (keyword)                                 | —                     | —      | —                 | —                                   | —                   |
| status (keyword)                                   | —                     | —      | —                 | —                                   | —                   |
...
- **Advanced builder** (`/search`): cross は共通 field のみ (横断可 = Tier1/2)。単一 DB scope を選ぶとその DB の DBごと field (Tier3) が候補に加わる。scope 切替は非破壊 (既存 condition の field は残し、scope 外は live sync が invalid を知らせる)。**facet 列を持つ field** (この表の `facet` が `—` でない field) は value 入力が editable な facet combobox になり、scope の facet 集計を候補に出す (件数付き、自由入力も可)。それ以外は free text 入力。
- **Sidebar cross** (`/search/results`、db 未指定): 横断可 (Tier1/2) の共通 field。organism / accessibility (facet) + name / publication (text) + datePublished / dateModified / dateCreated (range)。Tier3 は cross の `q` で弾かれるため出さない。
- **Sidebar per-DB** (`/search/results?db=<id>`): 共通 (organism / accessibility / submitter / name / publication / datePublished / dateModified / dateCreated。publication は biosample 除く) + その DB の DBごと field。sra / jga は subtype を絞る `type` facet を含む。
