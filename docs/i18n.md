...
`LangProvider` は root loaderData の決定値を React context として全 component に配る。 Provider 外で `useLang` / `useT` を呼ぶと throw する (SSR / CSR 双方で参照元を 1 つに固定するため)。
...
`GET /sitemap.xml` は content collection + 静的 routes について ja / en の 2 entry を出力し、 各 `<url>` 内に `xhtml:link` で相互宣言する。
...
`GET /robots.txt` は production のみ indexing 許可。 dev / staging では全 `Disallow: /` を返す。 実装は `server/api/robots.ts`。
