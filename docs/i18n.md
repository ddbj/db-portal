...
route module の `export const handle` の `i18n.en` に上表のいずれかを宣言する (SSOT は各 route file の `handle`)。 新規 page は `complete` を default にし、 en 訳の整備が間に合わない場合だけ `partial` / `missing` を宣言する。 宣言を省略した場合は banner 判定から除外されるため、 en 訳の整備状態を明示したい page では必ず宣言する。
