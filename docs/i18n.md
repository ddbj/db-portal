...
リクエストごとに 1 言語を決定する。 `?lang` hint は root loader が前段で処理して cookie 書き込み + 302 redirect で URL から取り除き、 残る 3 段 (cookie → Accept-Language → default) を純粋関数 `resolveLang()` で評価する。 最初に解決できた段で確定し、 後段は評価しない。 最終的に必ず `ja` または `en` のいずれかになる。
