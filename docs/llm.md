...
vLLM を内部 BFF 経由で利用する検索アシスタント。 ブラウザは vLLM の URL も API key も知らず、 BFF が SSE 中継・到達確認・rate limit・PII redaction だけを担う。 `/api/llm/search-assistant` の SSE protocol、 DSL pipeline (抽出 + parse + guards)、 health、 rate limit、 PII redaction を扱う。

## アーキテクチャ

ブラウザ → BFF → vLLM の 1 段中継。 BFF は vLLM への credential をブラウザに出さず、 モデルの逐次 delta も client へ転送しない (検証済み AST と DB slug が乗った 1 度の `done` event だけを送る)。 vLLM serving 自体 (GPU 割当・モデル起動) は別ライフサイクルの GPU node 側責務として `llm/README.md` に切り出す。 staging と production の app は同一 vLLM インスタンスを共有する。 ブラウザ ↔ BFF の一般則は [architecture.md](architecture.md) を参照。
...
BFF は open LLM proxy として振る舞わない。 LLM を使う新機能を増やすときは BFF endpoint と prompt を `/api/llm/<feature>` と `server/llm/<feature>/prompt.ts` の対で持ち、 client → BFF → vLLM の段を共通化する。

## /api/llm/search-assistant

自然言語 → DSL の SSE endpoint。 client が POST すると BFF が prompt を組んで vLLM に投げ、 モデル出力を DSL 検証して `done` event で AST と DB slug を返す。

### request body と Db スコープ
...
`mode` 省略時は `new`。 `append` のとき `current` 必須で BFF が DSL に serialize して prompt に差し込む。 `db` の有無で 2 mode に分かれ、 prompt の field 選択と DSL 検証経路が変わる ([search.md](search.md) § Sidebar facet の Tier 区分参照)。

- **locked** — body に `db` がある (per-DB ページ起点)。 cross と当該 DB の **Tier-3** を使い、 その DB で valid な DSL を返す
...
auto で eligible DB を選ぶときは problem `detail` の名指しを `DB_PRIORITY` (SSOT は `server/llm/assistant/search-api.ts`) でタイブレークする。 再 parse は 1 回限りで、 再び弾かれたら invalid に倒す。

### SSE response

response は `done` または `error` の **2 event のみ**。 `message` event (モデル逐次出力) は出さない。 生成中は `: heartbeat` だけを流し、 中継 proxy の idle timeout で接続が切れるのを防ぐ。
...
- `done.data.ast` は `/db-portal/parse` で検証済みの ParseNode、 `db` は確定 DB slug または `null` (cross 横断)
...
- stream open 直後に `: stream-open` を 1 度、 以降周期的に `: heartbeat` を流す。 間隔値は `server/llm/sse.ts`
- BFF rate-limit 超過は SSE 開始前に HTTP 429 で返す (詳細は § Rate limit)
- client (`useAssistantStream`) は `done` を proposal state へ、 `error` を inline エラー文言へ落とす

## DSL pipeline

vLLM 出力 (1 行の Advanced-Search DSL 文字列) を `done` に乗せるまでの pipeline。 抽出 → 決定論的修復 → parse を順に走らせる。 修復が失敗したら元の parse 結果に degrade する。
...
flowchart TD
  ...
  Clean --> Guard["決定論的修復 (Field availability + Subtype plane)"]
...
vLLM 出力からコードフェンス除去 + 先頭行抽出を行い、 ddbj-search-api `/db-portal/parse` で文法 allowlist と Tier-3 → DB 判定にかけて AST と確定 DB slug を得る。 BSI 側に field → DB マップは持たず、 backend の verdict を信頼する。
...
ddbj-search-api は一部 DB の Tier-1/2 cross field を index に持たない (SSOT は `_TIER12_UNAVAILABLE_DBS`)。 検索時は count=null として UI から落とすが、 parse は厳格で 400 `field-not-available-for-db` を返すため、 prompt で防ぎ切れないモデル prior 由来の取りこぼしを 0 件化させないよう、 locked / auto-解決済の db で非対応 cross field を出した top-level AND conjunct を **決定論的に落として** 同 db で 1 度だけ再 parse する。
...
sra / jga は subtype 別の独立 doc で、 異なる plane (sample / experiment / analysis / run / study 等) の field を AND すると 0 件になる ([search.md](search.md) § subtype plane)。 db が sra / jga に確定した AST が cross-plane のとき、 sample plane (organism 等) + cross field を残し、 experiment / analysis plane の field を落とし、 **落とした主 sequencing 概念を free-text 1 語として畳み込む**。
...
機能が使えるか・upstream に届くかの **3 状態** (`unset` / `ok` / `unreachable`) を返す endpoint。 状態は server in-memory に保持し、 handler は記録値を返すだけで request ごとに vLLM を叩かない。 response は `Cache-Control: no-store`、 schema の SSOT は `app/schemas/api-bff/llm.ts` (client / server 共用)。

初期状態は、 `DB_PORTAL_LLM_BASE_URL` が空なら即 `unset` に確定する。 設定済なら server 起動から 5 秒遅延後の最初の probe 結果で `ok` / `unreachable` に確定し、 以降は周期 probe で `ok` ↔ `unreachable` 間を遷移する。
...
- client (`useLlmAvailability` in `app/features/search/assistant/llm-availability.ts`) は TanStack Query で周期 polling し、 `ok` → `ready=true`、 `unset` → 機能 hide、 `unreachable` → 表示 + 送信時委譲を行う
...
user input は **素のまま prompt に組み立てて vLLM へ送る**。 redact が走るのは server log に出す直前のみで、 モデルに渡る入力は変えない。
...
env の定義 / 型 / default は `server/lib/env.ts` の Zod schema が SSOT、 値は `env.staging` / `env.production` 等が SSOT。 本表は BFF が読む env と GPU node の serving 側が読む env (`llm/compose.yml` + `entrypoint.sh`) の境界を示す。
