  ...
  "humandbs": {
    wizardSteps: {
      ja: [
        ...
        "必要書類を用いてデータ提供申請を作成し、提出する",
        "ヒトデータ審査委員会による審査・承認を待機（必要な書面が整ってから通常 2 週間程度）",
      ...
      en: [
        ...
        "Prepare the required documents and submit the data submission application",
        "Wait for review and approval by the Data Access Committee (typically about two weeks once the required documents are complete)",
    ...
    prepare: {
      ja: [
        ...
        "データ提供申請に必要な情報の準備（研究計画書、インフォームドコンセント文書、倫理審査承認書面、研究代表者・機関長の情報など）",
        "データの利用にあたってインフォームドコンセントの中で指定されている制限事項があるかの確認",
      ],
      en: [
        "DDBJ account (verified email address required)",
        "Information required for the data submission application (research plan, informed-consent documents, ethics-review approval, contact details for the principal investigator and head of institution, etc.)",
        "Confirmation of any restrictions specified in the informed-consent documents that apply to use of the data",
      ],
    },
    gotcha: { ja: "データ提供申請の審査結果は、通常、必要な書面が整ってから 2 週間程度で通知されますが、繁忙期や祝日等の状況により遅くなることがあります。承認されるまで JGA への登録は始められません。", en: "The data submission application is normally reviewed within about two weeks once the required documents are complete, though busy periods and holidays can extend this. JGA submission cannot begin until approval is granted." },
  ...
  "jga": {
    wizardSteps: {
      ja: [
        ...
        "NBDC ヒトデータベースにデータ提供申請 (申請システムで提供申請グループを作成し DDBJ アカウントで申請)。ヒトデータ審査委員会の承認後、JGA Submission ID とアップロード用ディレクトリが作成される (独自ポリシーが要る場合は別途 DBCLS に登録)",
        "提供申請の承認後、SFTP で登録用ディレクトリにメタデータ Excel・データファイルをアップロード",
      ...
      en: [
        ...
        "Submit a data submission application to the NBDC Human Database (create a data-submitter group and apply with your DDBJ account); approval by the Data Access Committee creates a JGA Submission ID and upload directory (register a custom policy with DBCLS if the NBDC standard policy does not apply)",
        "After approval, upload metadata Excel and data files to the submission directory via SFTP (port 443)",
    ...
    prepare: {
      ja: [
        "NBDC ヒトデータベースへのデータ提供申請の承認: ヒトデータ審査委員会による審査・承認が必要。審査結果は必要な書面が整ってから通常 2 週間程度で通知される",
      ...
      en: [
        "Approval of the data submission application to the NBDC Human Database by the Data Access Committee; review normally takes about two weeks once the required documents are complete",
    ...
    gotcha: { ja: "データのアップロードは NBDC ヒトデータベースの提供申請が承認された後にのみ可能で、審査結果は必要な書面が整ってから通常 2 週間程度で通知されます。", en: "Data upload is possible only after the data submission application to the NBDC Human Database is approved; review normally takes about two weeks once the required documents are complete." },
